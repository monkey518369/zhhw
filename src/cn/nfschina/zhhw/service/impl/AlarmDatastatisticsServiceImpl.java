...
public class AlarmDatastatisticsServiceImpl implements AlarmDatastatisticsService {
	...
	public Map<String, Object> getAlarmForCar(String id,Date startDate,Date endDate,String timeParty,List<String> types){
		...
		for(String str:types){//循环组织得到各个时段组织的垃圾重量
			...
			Date itemStartTime = new Date();//重新定义一个时间,作为循环用 的开始时间
			itemStartTime.setDate(startDate.getDate());
			itemStartTime.setMonth(startDate.getMonth());
			itemStartTime.setYear(startDate.getYear());
			itemStartTime.setHours(0);
			itemStartTime.setMinutes(0);
			itemStartTime.setSeconds(0);
