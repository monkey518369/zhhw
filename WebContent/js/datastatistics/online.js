...
function showcharts(){
	...
	if(type=="car"){
		...
		if(str.length<=1){
			...
			showCarCharts(type,ids,startDate,endDate,"column")
		}
		...
		if(str.length>1){
			...
			showCarCharts(type,ids,startDate,endDate,"spline");
		}
	}else{
		alert("未知错误,请重新尝试");
		location.reload(); 
...
//对车辆信息数据进行查询展示
function showCarCharts(type,ids,startDate,endDate,showType){
